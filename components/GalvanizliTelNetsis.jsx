...
const GalvanizliTelNetsis = () => {
  ...
  // Tolerans işaret değişikliği kontrolü ve değer takası
  useEffect(() => {
    // Eğer her iki işaret de negatif ise değerleri takası yap
    if (toleransMaxSign === '-' && toleransMinSign === '-') {
      ...
      // Değerler farklıysa takası yap
      if (plusValue !== minusValue) {
        setMmGtData(prevData => ({
          ...prevData,
          tolerans_plus: minusValue.toString(),
          tolerans_minus: plusValue.toString()
        }));
      }
    }
  }, [toleransMaxSign, toleransMinSign]);
  ...
  const generateStokAdi = () => {
    ...
    // Use actual tolerance signs from state
    const toleranceText = `${toleransMinSign}${Math.abs(toleransMinus).toFixed(2)}/${toleransMaxSign}${toleransPlus.toFixed(2)}`;
  ...
  const generateStokAdiForExcel = () => {
    ...
    // Use actual tolerance signs from state with comma format for Excel
    const toleranceText = `${toleransMinSign}${Math.abs(toleransMinus).toFixed(2).replace('.', ',')}/${toleransMaxSign}${toleransPlus.toFixed(2).replace('.', ',')}`;

...
const GalvanizliTelNetsis = () => {
  ...
  const calculateAutoRecipeValues = () => {
    ...
    allYmSts.forEach((ymSt, index) => {
      ...
      // Update the YM ST object with filmasin and quality values for dropdowns
      // This ensures the dropdowns show the correct automatic values
      const ymStCap = parseFloat(ymSt.cap) || cap;
      const filmasin = getFilmasinForCap(ymStCap);
      const quality = getQualityForCap(ymStCap);
      
      // Update the YM ST object directly to reflect in UI
      if (index < updatedSelectedYmSts.length) {
        updatedSelectedYmSts[index] = {
          ...updatedSelectedYmSts[index],
          filmasin: filmasin,
          quality: quality
        };
      } else {
        const autoIndex = index - updatedSelectedYmSts.length;
        updatedAutoGeneratedYmSts[autoIndex] = {
          ...updatedAutoGeneratedYmSts[autoIndex],
          filmasin: filmasin,
          quality: quality
        };
    ...
    // Update YM ST arrays with the new dropdown values
    setSelectedYmSts(updatedSelectedYmSts);
    setAutoGeneratedYmSts(updatedAutoGeneratedYmSts);

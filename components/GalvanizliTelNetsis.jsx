        ...
        if (ymSt.id) {
          ...
          if (recipeResponse && recipeResponse.ok) {
            ...
            if (recipes && recipes.length > 0) {
              // Update recipe input values with existing data
              const updatedRecipeValues = { ...recipeInputValues };
              
              recipes.forEach(recipe => {
                if (recipe.bilesen_kodu && recipe.miktar !== null && recipe.miktar !== undefined) {
                  updatedRecipeValues[`ymst_${recipe.bilesen_kodu}`] = recipe.miktar;
                }
              });
              
              setRecipeInputValues(updatedRecipeValues);
              console.log(`Loaded existing recipe data for YM ST ${ymSt.stok_kodu}:`, recipes.length, 'recipes');
            }
          }
        }
      }

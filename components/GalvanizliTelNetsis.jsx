...
const GalvanizliTelNetsis = () => {
  ...
  // The actual database save logic
  const continueSaveToDatabase = async (mmGtIds, ymGtId, ymStIds) => {
    try {
      ...
      // Original saveRecipesToDatabase logic
      // Sequence değeri MMGT ID'sinden değil, stok_kodu'ndan alınacak
      let sequence = '00'; // Varsayılan değer
    } catch (error) {
      console.error('Veri tabanına kayıt hatası:', error);
      toast.error('Veri tabanına kayıt yapılamadı: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveRecipesToDatabase = async (mmGtIds, ymGtId, ymStIds) => {
    // Save the parameters for use by continueSaveToDatabase
    const saveParams = { mmGtIds, ymGtId, ymStIds };

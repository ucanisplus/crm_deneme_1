...
const GalvanizliTelNetsis = () => {
  ...
  // Reçete verileri - Herbir YM ST için MM GT, YM GT ve YM ST reçeteleri
  const [allRecipes, setAllRecipes] = useState({
    mmGtRecipes: {}, // { ymStIndex: { recipe } }
    ymGtRecipe: {}, // Tek YM GT reçetesi
  ...
  const [activeRecipeType, setActiveRecipeType] = useState('mmgt'); // mmgt, ymgt, ymst
  ...
  const calculateAutoRecipeValues = (ymStIndex = null) => {
    ...
    if (ymStIndex !== null && ymStIndex < allYmSts.length) {
      // Belirli bir YM ST için reçeteler hesapla
      const ymSt = allYmSts[ymStIndex];
      
      // MM GT Reçete hesaplamaları
      const mmGtRecipeValues = {
        'GTPKT01': (10 / kg), // Paketleme operasyonu (DK)
        'AMB.ÇEM.KARTON.GAL': 8 / kg, // Karton (AD)
        [getShrinkCode(mmGtData.ic_cap)]: 1 / kg, // Shrink (KG) - dinamik kod
        'SM.7MMHALKA': 4 / kg, // Halka (AD)
        'AMB.APEX CEMBER 38X080': 1.2 / kg, // Çember (KG)
        'AMB.TOKA.SIGNODE.114P. DKP': 4 / kg, // Toka (AD)
        'SM.DESİ.PAK': calculateDesiConsumption(kg, cap), // Desi (AD)
        'naylon': 1 / kg // Naylon (KG)
      };
      
      // YM GT Reçete hesaplamaları
      const ymGtRecipeValues = {
        'GLV01': 1.15 - (0.125 * cap), // Galvanizleme operasyonu (DK)
        '150 03': (((1000 * 4000 / Math.PI / 7.85 / cap / cap * cap * Math.PI / 1000 * kaplama / 1000) + (5.54 * 0.6) + (2.73 * 0.7)) / 1000), // Çinko (KG)
        'SM.HİDROLİK.ASİT': calculateAcidConsumption(cap, kg, kaplama), // Asit (KG)
        [ymSt.stok_kodu]: 1 // YM ST tüketimi (KG)
      };
      
      // YM ST Reçete hesaplamaları
      const ymStCap = parseFloat(ymSt.cap) || cap;
      const ymStRecipeValues = {
        'TLC01': 0.2 / Math.pow(ymStCap, 1.7) + 0.02, // Tel çekme operasyonu (DK)
        [getFilmasinKodu(ymSt)]: 1 // Filmaşin tüketimi (KG)
      };
      
      setAllRecipes(prev => ({
        ...prev,
        mmGtRecipes: { ...prev.mmGtRecipes, [ymStIndex]: mmGtRecipeValues },
        ymGtRecipe: { ...prev.ymGtRecipe, ...ymGtRecipeValues },
        ymStRecipes: { ...prev.ymStRecipes, [ymStIndex]: ymStRecipeValues }
      }));
    } else {
      ...
      // İlk YM ST'yi YM GT reçetesine ekle
      if (allYmSts.length > 0) {
        newYmGtRecipe[allYmSts[0].stok_kodu] = 1;
      }
  ...
  const handleAddYmSt = () => {
    ...
    const capStr = parseFloat(newYmStData.cap).toFixed(2).padStart(4, '0').replace('.', '');
    const newYmSt = {
      ...
      cap: parseFloat(newYmStData.cap),
    ...
    const ymStIndex = selectedYmSts.length;
    calculateAutoRecipeValues(ymStIndex);
  };
  ...
  const saveToDatabase = async () => {
    try {
      ...
      // YM GT kontrolü ve kaydetme
      const ymGtStokKodu = `YM.GT.${mmGtData.kod_2}.${parseFloat(mmGtData.cap).toFixed(2).padStart(4, '0').replace('.', '')}.00`;
      ...
      for (let i = 0; i < allYmSts.length; i++) {
        ...
        // MM GT kontrolü
        const mmGtStokKodu = `GT.${mmGtData.kod_2}.${parseFloat(mmGtData.cap).toFixed(2).padStart(4, '0').replace('.', '')}.${sequence}`;
  ...
  // Stok Kartı Excel oluştur - çoklu MM GT ve YM ST destekli
  ...
  // Excel veri oluşturma fonksiyonları - doğru formatlar
  ...
  const generateYmStStokKartiData = (ymSt) => {
    const capFormatted = parseFloat(ymSt.cap).toFixed(4).replace('.', '').padStart(4, '0');
  ...
  const formatForExcel = (value, type) => {
    ...
    if (type === 'decimal' && (typeof value === 'number' || typeof value === 'string')) {
      // Sadece tolerans değerleri için virgülle format et
  ...
  const generateStokAdi = () => {
    ...
    const toleransPlus = mmGtData.tolerans_plus || 0;
    const toleransMinus = mmGtData.tolerans_minus || 0;
    
    return `Galvanizli Tel ${cap} mm -${Math.abs(toleransMinus).toFixed(2)}/+${toleransPlus.toFixed(2)} ${mmGtData.kaplama || '0'} gr/m²${mmGtData.min_mukavemet || '0'}-${mmGtData.max_mukavemet || '0'} MPa ID:${mmGtData.ic_cap || '45'} cm OD:${mmGtData.dis_cap || '75'} cm ${mmGtData.kg || '0'} kg`;
  };
  ...
  const generateEnglishName = () => {
    ...
    return `Galvanized Steel Wire ${cap} mm -${Math.abs(toleransMinus).toFixed(2)}/+${toleransPlus.toFixed(2)} ${mmGtData.kaplama || '0'} gr/m²${mmGtData.min_mukavemet || '0'}-${mmGtData.max_mukavemet || '0'} MPa ID:${mmGtData.ic_cap || '45'} cm OD:${mmGtData.dis_cap || '75'} cm ${mmGtData.kg || '0'} kg`;
  };
    ...
    <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
      ...
      {currentStep === 'summary' && (
        <div className="space-y-6">
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-sm text-gray-500 text-right">
                         <p>{parseFloat(ymSt.cap || 0).toFixed(2)} mm</p>
                          {selectedYmSts.find(item => item.stok_kodu === ymSt.stok_kodu) && (
          ...
          {/* Reçete Bölümü - Tabbed Interface */}
          {(selectedYmSts.length > 0 || autoGeneratedYmSts.length > 0) && (
            <div className="bg-white rounded-xl shadow-lg p-6">
              ...
              {/* Reçete Türü Sekmeleri */}
              {activeRecipeTab !== null && (
                <div className="space-y-6">
                  <div className="flex gap-2 mb-4">
                    {[
                      { key: 'mmgt', label: 'MM GT Reçete', color: 'red' },
                      { key: 'ymgt', label: 'YM GT Reçete', color: 'yellow' },
                      { key: 'ymst', label: 'YM ST Reçete', color: 'green' }
                    ].map((recipeType) => (
                      <button
                        key={recipeType.key}
                        onClick={() => setActiveRecipeType(recipeType.key)}
                        className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                          activeRecipeType === recipeType.key
                            ? `bg-${recipeType.color}-100 text-${recipeType.color}-700 border-2 border-${recipeType.color}-500`

...
const GalvanizliTelNetsis = () => {
  ...
  const fillEmptyFieldsWithAutoFill = () => {
    console.log('🔄 Otomatik Doldur: Boş alanlar doldurulacak...');
    
    // Update all recipes for all empty fields
    setAllRecipes(prev => {
      const updated = { ...prev };
      
      // Fill MM GT recipes for all YM STs
      [...selectedYmSts, ...autoGeneratedYmSts].forEach((ymSt, index) => {
        if (!updated.mmGtRecipes[index]) {
          updated.mmGtRecipes[index] = {};
        }
        
        // Define default values for MM GT recipe fields
        const mmGtDefaults = {
          'NAYLON': 0.001,
          'AMB.APEX CEMBER 38X080': 0.001,
          'AMB.TOKA.SIGNODE.114P. DKP': 0.004,
          'SM.7MMHALKA': 0.004,
          'AMB.ÇEM.KARTON.GAL': 0.008,
          'GTPKT01': 0.011,
          'SM.DESİ.PAK': 0.002
        };
        
        // Get shrink code based on diameter
        const shrinkCode = getShrinkCode(mmGtData.ic_cap);
        if (shrinkCode) {
          mmGtDefaults[shrinkCode] = 0.001;
        }
        
        Object.keys(mmGtDefaults).forEach(key => {
          if (!updated.mmGtRecipes[index][key] || updated.mmGtRecipes[index][key] === 0) {
            updated.mmGtRecipes[index][key] = mmGtDefaults[key];
          }
        });
      });
      
      // Fill YM GT recipe for all empty fields
      ...
      const ymGtDefaults = {
        'GLV01': 0.005,
        '150 03': 0.003,
        'SM.HİDROLİK.ASİT': 0.002
      };
      
      Object.keys(ymGtDefaults).forEach(key => {
        if (!updated.ymGtRecipe[key] || updated.ymGtRecipe[key] === 0) {
          updated.ymGtRecipe[key] = ymGtDefaults[key];
        }
      });
      
      // Fill YM ST recipes for all YM STs
      [...selectedYmSts, ...autoGeneratedYmSts].forEach((ymSt, index) => {
        if (!updated.ymStRecipes[index]) {
          updated.ymStRecipes[index] = {};
        }
        
        // Get filmasin code for this YM ST
        ...
        Object.keys(ymStDefaults).forEach(key => {
          if (!updated.ymStRecipes[index][key] || updated.ymStRecipes[index][key] === 0) {
            updated.ymStRecipes[index][key] = ymStDefaults[key];
          }
        });
      });
    ...
    console.log('✅ Otomatik Doldur tamamlandı - tüm boş alanlar dolduruldu');
  };
    ...
    try {
      ...
      if (allProductsResponse && allProductsResponse.ok) {
        ...
        if (allProducts.length > 0 && !isViewingExistingProduct) {
          ...
          console.log('Generated stok_adi:', mmGtStokAdi);
          console.log('Existing products stok_adi samples:', allProducts.slice(0, 3).map(p => p.stok_adi));
          
          const functionalDuplicates = allProducts.filter(product => {
            const isMatch = product.stok_adi === mmGtStokAdi;
            if (isMatch) {
              console.log('MATCH FOUND:', product.stok_kodu, product.stok_adi);
            }
            return isMatch;
          });
          ...
          if (functionalDuplicates.length > 0) {
            ...
            console.log(`ℹ️ Found product with same stok_kodu but different stok_adi - will create with incremented sequence`);
          }
        }
      }
    } catch (error) {
          ...
          if (!mainYmSt || !mainYmSt.stok_kodu) {
            ...
            if (!ymStEntry) {
              ...
              if (anyYmStEntry) {
                ...
                if (mainYmSt && mainYmSt.stok_kodu) {
                  console.log(`KRİTİK DÜZELTME: Eksik YM.ST bileşeni ekleniyor: ${mainYmSt.stok_kodu}`);
                  ...
                  <div className="p-6 bg-yellow-50 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      {[
                        ...
                        // For non-main YMSTs, show blank fields instead of calculated values
                        const isMainYmSt = activeRecipeTab === mainYmStIndex;
                        const currentValue = isMainYmSt ? (allRecipes.ymGtRecipe?.[key] || '') : '';

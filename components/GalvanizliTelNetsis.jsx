...
const GalvanizliTelNetsis = () => {
  ...
  const fetchRecipesFromDatabase = async () => {
    try {
      ...
      // Her MM GT için reçete kontrolü
  ...
  // İnkremental ürün oluşturma kontrolü
  const checkForExistingProducts = async (cap, kod_2, kaplama, minMukavemet, maxMukavemet, kg) => {
    try {
      ...
      if (response && response.ok) {
        ...
        // En yüksek sequence numarasını bul
        let maxSequence = -1;
  ...
  const saveToDatabase = async () => {
    try {
      ...
      // İnkremental numara kontrolü yap
      ...
      for (let i = 0; i < sequenceCount; i++) {
        const sequence = (nextSequence + i).toString().padStart(2, '0');
        const ymGtStokKodu = `YM.GT.${mmGtData.kod_2}.${capFormatted}.${sequence}`;
    ...
    try {
      ...
      if (ymGtId && Object.keys(allRecipes.ymGtRecipe).length > 0) {
        ...
        for (let i = 0; i < allYmSts.length; i++) {
          ...
          const ymStEntry = recipeEntries.find(([key]) => key.includes('YM.ST.') || key === allYmSts[i].stok_kodu);
          const operationEntry = recipeEntries.find(([key]) => key === 'GLV01');
          const otherEntries = recipeEntries.filter(([key]) => !key.includes('YM.ST.') && key !== 'GLV01' && key !== allYmSts[i].stok_kodu);
          
          // Sırayla ekle
          const orderedEntries = [
            ymStEntry ? [allYmSts[i].stok_kodu, ymStEntry[1]] : null,
            operationEntry,
            ...otherEntries
          ].filter(Boolean);

...
const GalvanizliTelNetsis = () => {
  ...
  // Update YM ST objects when switching tabs to ensure dropdowns show correct values
  useEffect(() => {
    if (selectedYmSts.length > 0 || autoGeneratedYmSts.length > 0) {
      ...
      if (activeYmSt && (activeYmSt.source === 'auto-generated' || activeYmSt.source === 'manual-added')) {
        // Ensure the active YM ST has proper filmasin and quality values
        const filmasinCode = getFilmasinKodu(activeYmSt);
        console.log(`Tab ${activeRecipeTab} activated: ${activeYmSt.stok_kodu}, Filmasin: ${filmasinCode}`);
        
        // Force re-render by updating the appropriate array
        setTimeout(() => {
          if (activeYmSt.source === 'auto-generated') {
            setAutoGeneratedYmSts(prev => [...prev]);
          } else if (activeYmSt.source === 'manual-added') {
            setSelectedYmSts(prev => [...prev]);
          }
        }, 0);
      }
    }

...
const GalvanizliTelNetsis = () => {
  ...
  const handleSelectExistingMmGt = async (mmGt) => {
    ...
    // Load all related data and recipes
    if (mmGt.id) {
      try {
        console.log('Loading comprehensive data for MM GT:', mmGt.id, mmGt.stok_kodu);
        ...
        // 1. Find related YM STs through the relationship table
          ...
          if (mmGtYmStRelations.length > 0) {
            ...
            for (const ymStId of uniqueYmStIds) {
              ...
              if (ymStResponse && ymStResponse.ok) {
                ...
                console.log(`Loaded YM ST: ${ymSt.stok_kodu}`);
              }
            }
            
            // Set the loaded YM STs
            setSelectedYmSts(loadedYmSts);
            
            // Set main YM ST index (first one by default)
            if (loadedYmSts.length > 0) {
              setMainYmStIndex(0);
            }
            
            // Now load all recipes in the correct structure
            ...
            if (mmGtRecipeResponse && mmGtRecipeResponse.ok) {
              ...
              // Initialize MM GT recipes for each YM ST
              loadedYmSts.forEach((_, index) => {
            ...
            // 3. Find and load YM GT data and recipes
            // First find the YM GT through the stok_kodu pattern
            ...
            // 4. Load YM ST recipes for each loaded YM ST
            ...
            // Update all recipe states at once
            setAllRecipes(updatedAllRecipes);
            setRecipeStatus(updatedRecipeStatus);
            
            console.log('All recipes loaded successfully');
            console.log('Recipe data:', updatedAllRecipes);
          }
        }
      } catch (error) {
        console.error('Error loading existing data and recipes:', error);
        toast.error('Mevcut ürün verileri yüklenirken hata oluştu');
      }
    }
    
    // Don't call findSuitableYmSts here as we already loaded the related YM STs
  ...
  const handleYmStSelection = async (ymSt) => {
    ...
    if (isSelected) {
      ...
      // Load recipe data for this YMST if it exists in database
      if (ymSt.id) {

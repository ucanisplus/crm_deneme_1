...
// Kolonları hem başlık metnine hem de veri özelliklerine göre tespit eden nihai algoritma
const findRelevantColumns = (data, headers) => {
  ...
  // İlk 15 satırdan örnek al
  const sampleRows = headers
    ? data.slice(1, Math.min(data.length, 16))
    : data.slice(0, Math.min(data.length, 15));
  ...
  // === 1. Adım: Başlık metnine göre öncelikli eşleşme ===

  const headerPriority = {
    hasirSayisi: ['hasir adedi', 'hasır adedi', 'adet', 'miktar', 'sayi'],
    uzunlukBoy: ['uzunluk boy', 'boy', 'boy uzunluk', 'boy cm', 'yukseklik'],
    uzunlukEn: ['uzunluk en', 'en', 'en uzunluk', 'en cm', 'genislik'],
    hasirTipi: ['hasir tipi', 'celik hasir', 'hasir', 'tip', 'hasir cinsi']
  };

  for (let i = 0; i < normalizedHeaders.length; i++) {
    const header = normalizedHeaders[i];

    for (const [key, keywords] of Object.entries(headerPriority)) {
      for (const keyword of keywords) {
        if (header.includes(keyword) && columnMap[key] === undefined) {
          columnMap[key] = i;
          break;
        }
      }
  ...
  for (let col = 0; col < sampleRows[0].length; col++) {
    ...
    for (const row of sampleRows) {
      ...
      const num = parseFloat(formatNumber(val));
    ...
    if (values.length > 0) {
      colStats[col] = {
        ...
        intRatio: values.filter(v => Number.isInteger(v)).length / values.length,
        boyRange: values.filter(v => v >= 275 && v <= 800).length / values.length,
        enRange: values.filter(v => v >= 100 && v <= 300).length / values.length
      };
    }
  }

  // === 3. Adım: Eksik kalan kolonları veriye göre tamamlama ===
  ...
  if (columnMap.hasirTipi === undefined) {
    ...
    for (let col = 0; col < sampleRows[0].length; col++) {
      ...
      for (const row of sampleRows) {
        const val = String(row[col] || '').trim();
        if (/^(Q|R|TR)\d+/i.test(val)) regexCounts[col]++;
      }
    }
    const best = Object.entries(regexCounts).sort((a, b) => b[1] - a[1])[0];
    if (best && best[1] > 0) columnMap.hasirTipi = parseInt(best[0]);
  }

  if (columnMap.uzunlukBoy === undefined) {
    const candidates = Object.entries(colStats)
      .filter(([_, stat]) => stat.boyRange >= 0.7)
      .sort((a, b) => b[1].boyRange - a[1].boyRange);
    if (candidates.length > 0) columnMap.uzunlukBoy = parseInt(candidates[0][0]);
  }

  if (columnMap.uzunlukEn === undefined) {
    const candidates = Object.entries(colStats)
      .filter(([_, stat]) => stat.enRange >= 0.7)
      .sort((a, b) => b[1].enRange - a[1].enRange);
  ...
  if (columnMap.hasirSayisi === undefined) {
    const candidates = Object.entries(colStats)
      .filter(([col, stat]) =>
        stat.intRatio > 0.9 &&
        stat.avg < 1000 &&
        ![columnMap.uzunlukBoy, columnMap.uzunlukEn, columnMap.hasirTipi].includes(parseInt(col))
      )
      .sort((a, b) => a[1].avg - b[1].avg);
    if (candidates.length > 0) columnMap.hasirSayisi = parseInt(candidates[0][0]);
  }
...
const calculateFilizValues = (row) => {
  ...
  // İyileştirme işlemlerini gerçekleştirme - Güvenilirlik için yeniden düzenlendi
  const iyilestir = async (rowIndex) => {
    try {
      ...
      // Temel değerler
      const hasirTipi = row.hasirTipi;
      let uzunlukBoy = parseFloat(row.uzunlukBoy);
      let uzunlukEn = parseFloat(row.uzunlukEn);
      let hasirSayisi = parseFloat(row.hasirSayisi);
      ...
      // ÖNEMLİ: Hasır türünü güncelle - İşlemin en başında yapılmalı
      row.hasirTuru = determineHasirTuru(row.hasirTipi, row.uzunlukBoy);
      ...
      if (!initialLimitsOk && uzunlukEn > MACHINE_LIMITS.MAX_EN) {
        ...
        if (isImproved) {
          ...
          // IMPORTANT: After swapping, update our local values
      ...
      if (!isImproved && hasirTipi.startsWith('Q')) {
        isImproved = trySwapBoyEn(row);
        if (isImproved) {
      ...
      if (!isImproved && !isMachineLimitsOk(row)) {
        ...
        if (isImproved) {
          // IMPORTANT: After multiplication, update our local values
          uzunlukBoy = parseFloat(row.uzunlukBoy);
          uzunlukEn = parseFloat(row.uzunlukEn);
          hasirSayisi = parseFloat(row.hasirSayisi);
          
          // ÖNEMLİ: Değerler değiştiyse hasır türünü yeniden belirle
          row.hasirTuru = determineHasirTuru(row.hasirTipi, row.uzunlukBoy);
        }
        // tryMultiplyDimensions içinde açıklama güncellenir
      }
      
      // ÖNEMLİ: Çubuk sayılarını burada hesapla - Boy/En işlemleri tamamlandıktan sonra
      initializeCubukSayisi(row);
      
      // 4. Filiz değerlerini optimize et
      if (!row.uretilemez) {
        ...
        const initialScore = scoreFiliz(row, { on: initialFiliz.on, arka: initialFiliz.arka });
        const optimizedScore = scoreFiliz(row, { on: optimizedFiliz.on, arka: optimizedFiliz.arka });
...
const iyilestirAll = async () => {
  ...
  try {
    ...
    for (const rowIndex of eligibleRowIndexes) {
      ...
      // 1. Öncelikle En > MAX_EN kontrolü - Önemli işlem
      ...
      // 2. Q tipi için Boy/En değiştirmeyi dene - Eğer En > MAX_EN için yapılmadıysa
      ...
      // Çubuk sayılarının hesaplanmasını sağla - Boy/En işlemleri tamamlandıktan sonra
      ...
      if (oldCubukSayisiBoy !== row.cubukSayisiBoy || oldCubukSayisiEn !== row.cubukSayisiEn) {
        changesCount++;
        newAciklama += `7. Çubuk sayıları hesaplandı (Boy: ${row.cubukSayisiBoy}, En: ${row.cubukSayisiEn}). `;
      }
      
      // 4. Filiz değerlerini hesapla ve optimize et - Tüm Boy/En işlemleri sonrası
      if (!row.uretilemez) {
        // Filiz değerlerini hesaplayıp kaydet
        ...
        // Filiz skorlama mantığını uygula
        const scoreFiliz = (row, filiz) => {
          ...
          // Rule 1: Q type, Döşeme → strictly enforce 15-22 range
          if (row.hasirTipi.startsWith('Q') && row.hasirTuru === 'Döşeme') {
              // İdeal aralık 15-22
              if (on < 15 || on > 22 || arka < 15 || arka > 22) {
                  return -100; // Aralık dışındaysa büyük ceza
              }
              
              // İdeal değerlere verilen bonus
              let idealBonus = 0;
              
              // Değer 17.5'e yakın mı?
              if (Math.abs(on - 17.5) < 1) idealBonus += 50;
              if (Math.abs(arka - 17.5) < 1) idealBonus += 50;
              
              return 500 + idealBonus;
          }
          
          // Rule 2: Perde → prefer ön < 16, arka = 70
          if (row.hasirTipi.startsWith('Q') && (row.hasirTuru === 'Perde' || row.hasirTuru === 'DK Perde')) {
              let score = 0;
              
              // Ön filiz için puan (15'e yakın ve 16'dan küçük olmalı)
              if (on < 16) score += 100;
              if (Math.abs(on - 15) < 1) score += 50;
              
              // Arka filiz puanlaması (70 > 75 > 65)
              if (Math.abs(arka - 70) < 1) score += 150;
              else if (Math.abs(arka - 75) < 1) score += 100;
              else if (Math.abs(arka - 65) < 1) score += 50;
              
              // 5'in katı artık daha az önemli
              if (Math.abs(arka % 5) < 0.1) score += 10;
              
              return score;
          }
          
          // Rule 3: Other Q types 
          if (row.hasirTipi.startsWith('Q')) {
        ...
        // Daha iyi olan değerleri kullan
        if (initialScore > optimizedScore) {
          row.onFiliz = initialFiliz.on;
          row.arkaFiliz = initialFiliz.arka;
          row.modified.onFiliz = true;
        ...
        if (Math.abs(oldFilizValues.solFiliz - row.solFiliz) > 0.1 || 
          ...
          changesCount++;
          newAciklama += `9. Filiz değerleri optimize edildi (Sol/Sağ: ${row.solFiliz.toFixed(2)}cm, Ön: ${row.onFiliz.toFixed(2)}cm, Arka: ${row.arkaFiliz.toFixed(2)}cm). `;
          ...
          if ((row.hasirTuru === 'Perde' || row.hasirTuru === 'DK Perde') && 
              (Math.abs(row.arkaFiliz - 70) < 1 || Math.abs(row.arkaFiliz - 75) < 1 || Math.abs(row.arkaFiliz - 65) < 1)) {
            newAciklama += `Arka filiz değeri ${row.arkaFiliz.toFixed(0)}cm olarak optimize edildi. `;
          }
        }
...
// Excel dosyasını işleyip gerekli kolonları çıkarır, her satırın ait olduğu sayfayı da belirtir
const parseExcelData = (data) => {
  try {
    const workbook = XLSX.read(data, { type: 'array' });
    const allSheetData = [];

    // Tüm sayfaları sırayla işle
    for (let sheetIndex = 0; sheetIndex < workbook.SheetNames.length; sheetIndex++) {
      ...
      // Geçerli veri satırlarını filtrele
      ...
      // Sayfanın ilk satırında birleştirilmiş hücre başlığı varsa düzelt
      ...
      // Kolon tespit algoritmaları
      const directMap = findColumnsByHeaderText(headerRow);
      const fallbackMap = findRelevantColumns(jsonData, headerRow);
      const columnStats = analyzeColumnData(jsonData, hasHeaders);
      const columnMap = combineColumnMaps(directMap, fallbackMap, columnStats);
      ...
      for (let rowIndex = startRow; rowIndex < jsonData.length; rowIndex++) {
        ...
        if (!hasirTipi || !/^(Q|R|TR)\d+/i.test(hasirTipi)) continue;

        // Boy, En, Adet verilerini al
        let uzunlukBoy = columnMap.uzunlukBoy !== undefined && columnMap.uzunlukBoy < row.length
          ? String(row[columnMap.uzunlukBoy] || '').trim()
          : '';

        let uzunlukEn = columnMap.uzunlukEn !== undefined && columnMap.uzunlukEn < row.length
          ? String(row[columnMap.uzunlukEn] || '').trim()
          : '';

        let hasirSayisi = columnMap.hasirSayisi !== undefined && columnMap.hasirSayisi < row.length
          ? String(row[columnMap.hasirSayisi] || '').trim()
          : '';
        ...
        // Adet yoksa varsayılan olarak 1 ata
        if (!hasirSayisi) {
          hasirSayisi = '1';
        }

        // Boy veya En varsa veriyi kaydet
    ...
    // Ön izleme verilerini hazırla
    const previewItems = allSheetData.map((row, index) => ({
      id: index,
      hasirTipi: row.hasirTipi || '',
      uzunlukBoy: row.uzunlukBoy || '',
      uzunlukEn: row.uzunlukEn || '',
      hasirSayisi: row.hasirSayisi || '',
      aciklama: `Sayfa: ${row.sheetName}`
    }));
...
// Başlık metinlerine göre kolonları bulur, özel olarak bazı başlıkları önceliklendirir
const findColumnsByHeaderText = (headers) => {
  ...
  // Öncelikli başlıklar (önce gelen önce atanır)
  const headerPatterns = {
    hasirTipi: [
      'HASIR ADEDİ', 'HASIR ADEDI', // özel tür değil, yanlışlıkla bu kategoriye girerse sıralamaya dikkat
      'HASIR TİP', 'HASIR TIP', 'ÇELIK HASIR', 'CELIK HASIR', 'TİP',
      'HASIR CİNS', 'HASIR CINS', 'HASIR', 'ÇELİK TİP', 'CELIK TIP'
    ],
    uzunlukBoy: [
      'UZUNLUK BOY', 'BOY', 'BOY UZUNLUK', 'YÜKSEKLIK', 'YUKSEKLIK', 'BOY CM',
      'Y.BOY', 'YÜKSEKLİK (CM)', 'YÜKSEKLIK (CM)'
    ],
    uzunlukEn: [
      'UZUNLUK EN', 'EN', 'EN UZUNLUK', 'GENISLIK', 'GENİŞLİK', 'EN CM',
      'Y.EN', 'GENİŞLİK (CM)', 'GENISLIK (CM)'
    ],
    hasirSayisi: [
      'HASIR ADEDİ', 'HASIR ADEDI', // en güçlü eşleşme başta
      'HASIR SAYISI', 'HASIR SAYIS', 'ADET', 'MİKTAR', 'MIKTAR',
      'SAYI', 'SİPARİŞ ADEDİ', 'SIPARIS ADEDI', 'TOPLAM ADET',
      'TOPLAM', 'ADET SAYISI', 'ADET MİKTARI', 'ADET MIKTARI',
      'SİPARİŞ', 'SIPARIS'
    ]
  };

  // Tüm başlıklar için karşılaştırma yap
  for (let i = 0; i < headers.length; i++) {
    ...
    for (const [category, patterns] of Object.entries(headerPatterns)) {
      for (const pattern of patterns) {
        if (header === pattern && result[category] === undefined) {
          result[category] = i;
          break;
        }
      }
      ...
      for (const pattern of patterns) {
        if (header.includes(pattern) && result[category] === undefined) {
          result[category] = i;
          break;
        }
      }
...
// Kolon verilerini analiz eder, özellikle hasır sayısı olabilecek sütunu belirlemeye çalışır
const analyzeColumnData = (data, hasHeaders) => {
  ...
  // En fazla 20 satır üzerinden örnekleme
  ...
  // Hasır Sayısı olabilecek en iyi sütunu tespit et
  let bestScore = -1;
  ...
  for (const [col, stats] of Object.entries(result.columnStats)) {
    ...
    if (stats.minValue >= 1 && stats.maxValue <= 100000) score += 30;
    if (stats.integerCount / stats.numericCount > 0.9) score += 30;
    if (stats.avgValue < 100) score += 20;
    ...
    if (!stats.hasQRTRPattern) score += 50;
  ...
  // Belirli eşiği geçerse hasır sayısı olarak ata
  if (bestScore >= 60) {
    result.potentialHasirSayisi = bestCol;
  }
...
// Doğrudan eşleşmeler, veri analizi ve fallback eşleşmeleri birleştirilir
const combineColumnMaps = (directMap, fallbackMap, columnCharacteristics) => {
  ...
  // 1. Öncelik: Başlık eşleşmeleri (directMap)
  ...
  // 2. Hasır Sayısı için: Kolon karakteristiklerine göre analiz
  ...
  // 3. Eksik kalanlar için fallback eşleşmeleri
...
// CSV dosyalarını işleme - Aynı gelişmiş analiz ile
  ...
  try {
    Papa.parse(data, {
      ...
      complete: (results) => {
        ...
        // Excel işleme kodu ile benzer mantık
        ...
        for (let rowIndex = startRow; rowIndex < jsonData.length; rowIndex++) {
          ...
          // Hasır tipi var mı kontrol et
          let hasirTipi = '';
          if (columnMap.hasirTipi !== undefined && columnMap.hasirTipi < row.length) {
            hasirTipi = String(row[columnMap.hasirTipi] || '').trim();
          }
          
          // Eğer belirtilen sütunda bulunamadıysa, tüm hücrelerde ara
          if (!hasirTipi || !/^(Q|R|TR)\d+/i.test(hasirTipi)) {
            for (const cell of row) {
              if (!cell) continue;
              const cellValue = String(cell).trim();
          ...
          // Diğer değerleri kolon haritasına göre çıkar
          let uzunlukBoy = '';
          let uzunlukEn = '';
          let hasirSayisi = '';
          ...
          // Hasır sayısı yoksa ve açıkça bir sayı sütunu varsa bak
          if (!hasirSayisi && columnCharacteristics.potentialHasirSayisi) {
          ...
          // Hala yoksa varsayılan olarak 1 kullan
          if (!hasirSayisi) {
            hasirSayisi = '1';
          }
          
          // Boyutları doğrula (en az biri olmalı)
          if (uzunlukBoy || uzunlukEn) {
            validRows.push({
              hasirTipi: standardizeHasirTipi(hasirTipi),
              uzunlukBoy: uzunlukBoy,
              uzunlukEn: uzunlukEn,
              hasirSayisi: hasirSayisi,
              sheetName: "CSV" // CSV için standart sayfa adı
            });
          }
        }

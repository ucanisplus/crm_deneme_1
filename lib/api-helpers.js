...
function sanitizeDataForApi(data) {
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      // Handle timestamp fields and ensure they're in the correct format for PostgreSQL timestamptz
      if ((key.includes('_update') || key.includes('_tarihi') || key.endsWith('_at') || key.includes('Date'))) {
        ...
        // Special handling for year values like "2025" that cause timestamp errors
        if (typeof value === 'string' && /^\d{4}$/.test(value)) {
          const year = parseInt(value);
          if (year >= 1900 && year <= 2100) {
            // Full ISO8601 format with Z for PostgreSQL timestamptz compatibility
            result[key] = `${year}-01-01T00:00:00.000Z`;
            console.log(`Converted year value ${value} to ISO8601 timestamp ${result[key]} for field ${key}`);
          } else {
            result[key] = null;
          }
          continue;
        ...
        // Handle Date objects and valid date strings
        if (value instanceof Date || (typeof value === 'string' && isValidDate(value))) {
          try {
            // Make sure we get a proper Date object
            const dateObj = value instanceof Date ? value : new Date(value);
            // Convert to ISO8601 format with Z
            result[key] = dateObj.toISOString();
            console.log(`Formatted date for field ${key}: ${result[key]}`);
          } catch (error) {
            console.error(`Error formatting timestamp for field ${key}:`, error);
            result[key] = null;
          }
          continue;
        }
        
        // If we get here with an unprocessed value, set to null for safety
        console.warn(`Unrecognized timestamp format for field ${key}:`, value);
        result[key] = null;
        continue;
      }

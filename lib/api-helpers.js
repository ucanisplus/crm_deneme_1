...
function sanitizeDataForApi(data) {
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      // Handle timestamp fields and ensure they're in the correct format
      if ((key.includes('_update') || key.includes('_tarihi') || key.endsWith('_at') || key.includes('Date'))) {
        // Use our safe timestamp formatter for date fields
        if (value === null || value === undefined) {
          ...
          if (year >= 1900 && year <= 2100) {
            // Format with timezone for PostgreSQL timestamptz
            result[key] = `${year}-01-01T00:00:00.000Z`;
          } else {
            result[key] = null;
          }
        } else {
          // For safety, convert unknown values to null
          result[key] = null;
        }
        continue;
      }

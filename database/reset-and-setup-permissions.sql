-- Complete Reset and Setup of Permissions System
-- WARNING: This will DELETE all existing permissions!

-- 1. First, backup existing permissions (optional - view current data)
SELECT * FROM user_permissions ORDER BY role, permission_name;

-- 2. Delete all existing permissions
DELETE FROM user_permissions;

-- 3. Reset the auto-increment sequence (if using SERIAL)
-- Find the sequence name first
SELECT 
    column_default 
...
-- Reset sequence (adjust the sequence name if different)
-- Common names: user_permissions_id_seq or user_permissions_pkey_seq
ALTER SEQUENCE user_permissions_id_seq RESTART WITH 1;

-- 4. Insert all permissions fresh
...
-- Fabrika Müdürü (production + limited maliyet)
INSERT INTO user_permissions (role, permission_name) VALUES
...
-- muhendis_2 (based on your screenshot)
INSERT INTO user_permissions (role, permission_name) VALUES
...
-- 6. Check specific role permissions
-- Example: Check what admin can access
